...
$check = mysqli_query($conn,"SELECT * FROM `pharmacytbl` WHERE `userName` = '$username'");
    ...
    if(mysqli_num_rows($check)<1){
        ...
        if(password_verify($password, $row['password'])){
            if (mysqli_query($conn, $sql1)) {
                ...
                $_SESSION['name'] =  $row['pharmacyName'];
...
if($_POST['type'] == 2){
    ...
    $ip_address = $_SERVER['REMOTE_ADDR'];
     ...
     $sql = "UPDATE `pharmacytbl` SET `userName`='$username',`password`='$password' WHERE `pharmacyId`='$id'";
    $sql1 = "INSERT INTO `logstbl`(`logs`, `ipaddress`, `date`) VALUES (CONCAT('$username',' changed password!'),'$ip_address','$datenow')";
...
$sql = "UPDATE `pharmacytbl` SET `userName`='$username',`password`='$password' ,`pharmacyName`='$name',`email`='$email',`contactNumber`='$contact',`address`='$address',`barangayId`='$barangay',`city`='$city',`province`='$province',`dateModified`='$datenow' WHERE `pharmacyId`='$id'";
$sql1 = "INSERT INTO `logstbl`(`logs`, `ipaddress`, `date`) VALUES (CONCAT('$username',' update pharmacy details.'),'$ip_address','$datenow')";
	if (mysqli_query($conn, $sql)) {
        if (mysqli_query($conn, $sql1)) {
			echo json_encode(array("statusCode"=>200));
        }
...
$datenow =  date("Y-m-d h:i:sa");
$ip_address = $_SERVER['REMOTE_ADDR'];
$sql = "DELETE FROM `productstbl` WHERE `productId`='$id'";
$sql1 = "INSERT INTO `logstbl`(`logs`, `ipaddress`, `date`) VALUES (CONCAT('$name',' delete a product.'),'$ip_address','$datenow')";
    if (mysqli_query($conn, $sql)) {
        if (mysqli_query($conn, $sql1)) {
            echo json_encode(array("statusCode"=>200));
        }
...
if($_POST['type'] == 5){
$id = $_POST['id']; 
$sql = mysqli_query($conn,"SELECT * FROM productstbl WHERE productId = '$id'");
    if(mysqli_num_rows($sql)>0){
        ...
        <div class="row">
            <div class="col-md-5 ">
                ...
                <img src="../images/'.$row['productImage'].'" alt="Image" id="images" class="img-fluid p-1" style="max-width: 60% !important;">
                </div>
            </div>
            <div class="col-md-7">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group my-2">
                            <label for="brand_name" class="text-uppercase">Brand Name</label>
                            <input class="form-control text-uppercase" id="brand_name" 
                            value="'.$row['brandName'].'" disabled/>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group my-2">
                            <label for="generic_name" class="text-uppercase">Generic Name</label>
                            <input class="form-control text-uppercase" id="generic_name" value="'.$row['genericName'].'" disabled />
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group my-2">
                            <label for="prescription" class="text-uppercase">Prescription</label>
                            <input class="form-control text-uppercase" id="prescription" value="'.$row['drugType'].'" disabled />
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group my-2">
                            <label for="price" class="text-uppercase">Price</label>
                            <input class="form-control" id="price" value="'.$row['price'].'" disabled />
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group my-2">
                            <label for="stock" class="text-uppercase">Stock/s</label>
                            <input class="form-control" id="stock" value="'.$row['stock'].'" disabled />
                        </div>
                    </div>
                    <div class="col-sm-12">
                        <div class="form-group my-2">
                            <label for="description" class="text-uppercase">Description </label>
                            <textarea class="form-control text-uppercase" id="description" disabled>'.$row['description'].'</textarea>
                        </div>
                    </div>
                </div>
            </div>
